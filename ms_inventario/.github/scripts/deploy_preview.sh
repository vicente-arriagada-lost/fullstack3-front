...
ensure_namespace() {
  ...
  operation_id="$(aws servicediscovery create-private-dns-namespace \
    ...
    --vpc "$VPC_ID" \
    --creator-request-id "inventario-pr-${PR_NUMBER}-namespace" \
    --query 'OperationId' \
    --output text)"
...
patch_microservice_task_definition() {
  jq -c \
    --arg container "$INVENTARIO_CONTAINER_NAME" \
    --arg image "$IMAGE_URI" \
    --arg node_env "pr-${PR_NUMBER}" \
    --arg schema "pr_${PR_NUMBER}" \
    --arg mongodb_database "smartlogix_inventario_pr_${PR_NUMBER}" '
        ...
        if .name == $container then
          .environment = ((.environment // [])
            | map(select(
                ...
                and .name != "KONG_INVENTARIO_STABLE_WEIGHT"
                and .name != "KONG_INVENTARIO_CANARY_WEIGHT"
              ))
            + [
                {"name":"KONG_SERVICE_DISCOVERY_NAMESPACE","value":$namespace},
                {"name":"KONG_INVENTARIO_STABLE_WEIGHT","value":"100"},
                {"name":"KONG_INVENTARIO_CANARY_WEIGHT","value":"0"}
              ])
        else
          .
        end
...
INVENTARIO_PREFIX="${INVENTARIO_DEPLOY_CONTRACT_PREFIX:-/smartlogix/inventario/deploy}"
KONG_PREFIX="${KONG_DEPLOY_CONTRACT_PREFIX:-/smartlogix/kong/deploy}"

PREVIEW_HOST_TEMPLATE="${PREVIEW_HOST_TEMPLATE:-pr-%s.inventario-preview.internal}"
PREVIEW_HOST="$(printf "$PREVIEW_HOST_TEMPLATE" "$PR_NUMBER")"
PREVIEW_NAMESPACE="smartlogix-pr-${PR_NUMBER}.local"
INVENTARIO_SERVICE_NAME="srv-inventario-pr-${PR_NUMBER}"
INVENTARIO_TASK_FAMILY="inventario-pr-${PR_NUMBER}"
KONG_SERVICE_NAME="srv-kong-inventario-pr-${PR_NUMBER}"
KONG_TASK_FAMILY="kong-inventario-pr-${PR_NUMBER}"
KONG_TARGET_GROUP_NAME="tg-kinvpr-${PR_NUMBER}"

CLUSTER_NAME="$(get_param "$INVENTARIO_PREFIX" cluster_name)"
PROD_INVENTARIO_SERVICE_NAME="$(get_param "$INVENTARIO_PREFIX" service_name)"
INVENTARIO_CONTAINER_NAME="$(get_param "$INVENTARIO_PREFIX" container_name)"
INVENTARIO_CONTAINER_PORT="$(get_param "$INVENTARIO_PREFIX" container_port)"
...
INVENTARIO_DISCOVERY_SERVICE_ARN="$(ensure_discovery_service inventario)"
ensure_discovery_service inventario-canary >/dev/null

PROD_INVENTARIO_TASK_DEFINITION_ARN="$(aws ecs describe-services --cluster "$CLUSTER_NAME" --services "$PROD_INVENTARIO_SERVICE_NAME" --query 'services[0].taskDefinition' --output text)"
BASE_INVENTARIO_TASK_DEFINITION_JSON="$(aws ecs describe-task-definition --task-definition "$PROD_INVENTARIO_TASK_DEFINITION_ARN" --query 'taskDefinition' --output json)"
INVENTARIO_TASK_DEFINITION_JSON="$(patch_microservice_task_definition <<<"$BASE_INVENTARIO_TASK_DEFINITION_JSON")"
INVENTARIO_REGISTER_PAYLOAD="$(build_register_payload "$INVENTARIO_TASK_DEFINITION_JSON" "$INVENTARIO_TASK_FAMILY")"
INVENTARIO_TASK_DEFINITION_ARN="$(aws ecs register-task-definition --cli-input-json "$INVENTARIO_REGISTER_PAYLOAD" --query 'taskDefinition.taskDefinitionArn' --output text)"
...
INVENTARIO_STATUS="$(aws ecs describe-services --cluster "$CLUSTER_NAME" --services "$INVENTARIO_SERVICE_NAME" --query 'services[0].status' --output text 2>/dev/null || true)"
if [[ "$INVENTARIO_STATUS" == "ACTIVE" ]]; then
  aws ecs update-service \
    --cluster "$CLUSTER_NAME" \
    --service "$INVENTARIO_SERVICE_NAME" \
    --task-definition "$INVENTARIO_TASK_DEFINITION_ARN" \
    --desired-count 1 \
  ...
  aws ecs create-service \
    --cluster "$CLUSTER_NAME" \
    --service-name "$INVENTARIO_SERVICE_NAME" \
    --task-definition "$INVENTARIO_TASK_DEFINITION_ARN" \
    --desired-count 1 \
    --launch-type FARGATE \
    --network-configuration "$NETWORK_CONFIGURATION" \
    --service-registries "registryArn=${INVENTARIO_DISCOVERY_SERVICE_ARN}" >/dev/null
fi
...
aws ecs wait services-stable --cluster "$CLUSTER_NAME" --services "$INVENTARIO_SERVICE_NAME" "$KONG_SERVICE_NAME"
