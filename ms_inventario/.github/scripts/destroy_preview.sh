...
INVENTARIO_TASK_DEFINITION_ARN="$(aws ecs describe-services --cluster "$CLUSTER_NAME" --services "$INVENTARIO_SERVICE_NAME" --query 'services[0].taskDefinition' --output text 2>/dev/null || true)"
run_schema_cleanup "$INVENTARIO_TASK_DEFINITION_ARN"
...
delete_ecs_service_if_active "$INVENTARIO_SERVICE_NAME"
...
deregister_task_definitions "$INVENTARIO_TASK_FAMILY"

NAMESPACE_ID="$(find_namespace_id || true)"
for service_name in inventario inventario-canary; do
