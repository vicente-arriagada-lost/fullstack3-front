...
patch_inventario_task_definition() {
  jq -c \
    --arg container "$INVENTARIO_CONTAINER_NAME" \
    --arg image "$IMAGE_URI" \
    --arg aws_region "$AWS_REGION" \
    --arg mongodb_database "smartlogix_inventario" \
  ...
  jq -c \
    ...
    --arg canary_weight "$canary_weight" '
      .containerDefinitions |= map(
        if .name == $container then
          .environment = ((.environment // [])
            | map(select(.name != "KONG_INVENTARIO_STABLE_WEIGHT" and .name != "KONG_INVENTARIO_CANARY_WEIGHT"))
            + [
                {"name":"KONG_INVENTARIO_STABLE_WEIGHT","value":$stable_weight},
                {"name":"KONG_INVENTARIO_CANARY_WEIGHT","value":$canary_weight}
              ])
        else
          .
        end
...
smoke_test_canary_route() {
  ...
  status_code="$(curl -sS -o /dev/null -w '%{http_code}' \
    ...
    --connect-timeout 5 \
    --max-time 15 \
    "http://${alb_dns}/api/inventario")"

  if [[ "$status_code" != "200" ]]; then
    echo "Smoke test failed. Expected 200 from inventario service, got ${status_code}." >&2
    exit 1
  fi
}
...
STABLE_SERVICE_NAME="$(get_param "$INVENTARIO_PREFIX" service_name)"
CANARY_SERVICE_NAME="$(get_param "$INVENTARIO_PREFIX" canary_service_name)"
INVENTARIO_CONTAINER_NAME="$(get_param "$INVENTARIO_PREFIX" container_name)"
INVENTARIO_TASK_FAMILY="$(get_param "$INVENTARIO_PREFIX" task_definition_family)"
CANARY_DISCOVERY_SERVICE_ARN="$(get_param "$INVENTARIO_PREFIX" canary_discovery_service_arn)"
EVENTS_TOPIC_ARN="$(get_param "$INVENTARIO_PREFIX" events_topic_arn)"
...
PATCHED_INVENTARIO_TASK_DEFINITION_JSON="$(patch_inventario_task_definition <<<"$BASE_STABLE_TASK_DEFINITION_JSON")"
CURRENT_STABLE_IMAGE="$(jq -r --arg container "$INVENTARIO_CONTAINER_NAME" '.containerDefinitions[] | select(.name == $container) | .image' <<<"$BASE_STABLE_TASK_DEFINITION_JSON")"

if [[ "$CURRENT_STABLE_IMAGE" == "alpine:latest" ]]; then
  STABLE_REGISTER_PAYLOAD="$(build_register_payload "$PATCHED_INVENTARIO_TASK_DEFINITION_JSON" "$INVENTARIO_TASK_FAMILY")"
...
CANARY_REGISTER_PAYLOAD="$(build_register_payload "$PATCHED_INVENTARIO_TASK_DEFINITION_JSON" "inventario-canary-main")"
