...
ensure_namespace() {
  ...
  operation_id="$(aws servicediscovery create-private-dns-namespace \
    ...
    --vpc "$VPC_ID" \
    --creator-request-id "envios-pr-${PR_NUMBER}-namespace" \
    --query 'OperationId' \
    --output text)"
...
patch_microservice_task_definition() {
  jq -c \
    --arg container "$ENVIOS_CONTAINER_NAME" \
    --arg image "$IMAGE_URI" \
        ...
        if .name == $container then
          .environment = ((.environment // [])
            | map(select(
                ...
                and .name != "KONG_ENVIOS_STABLE_WEIGHT"
                and .name != "KONG_ENVIOS_CANARY_WEIGHT"
              ))
            + [
                {"name":"KONG_SERVICE_DISCOVERY_NAMESPACE","value":$namespace},
                {"name":"KONG_ENVIOS_STABLE_WEIGHT","value":"100"},
                {"name":"KONG_ENVIOS_CANARY_WEIGHT","value":"0"}
              ])
        else
          .
        end
...
PROD_ENVIOS_SERVICE_NAME="$(get_param "$ENVIOS_PREFIX" service_name)"
ENVIOS_CONTAINER_NAME="$(get_param "$ENVIOS_PREFIX" container_name)"
ENVIOS_CONTAINER_PORT="$(get_param "$ENVIOS_PREFIX" container_port)"
...
ENVIOS_DISCOVERY_SERVICE_ARN="$(ensure_discovery_service envios)"
ensure_discovery_service envios-canary >/dev/null

PROD_ENVIOS_TASK_DEFINITION_ARN="$(aws ecs describe-services --cluster "$CLUSTER_NAME" --services "$PROD_ENVIOS_SERVICE_NAME" --query 'services[0].taskDefinition' --output text)"
BASE_ENVIOS_TASK_DEFINITION_JSON="$(aws ecs describe-task-definition --task-definition "$PROD_ENVIOS_TASK_DEFINITION_ARN" --query 'taskDefinition' --output json)"
ENVIOS_TASK_DEFINITION_JSON="$(patch_microservice_task_definition <<<"$BASE_ENVIOS_TASK_DEFINITION_JSON")"
ENVIOS_REGISTER_PAYLOAD="$(build_register_payload "$ENVIOS_TASK_DEFINITION_JSON" "$ENVIOS_TASK_FAMILY")"
ENVIOS_TASK_DEFINITION_ARN="$(aws ecs register-task-definition --cli-input-json "$ENVIOS_REGISTER_PAYLOAD" --query 'taskDefinition.taskDefinitionArn' --output text)"
...
ENVIOS_STATUS="$(aws ecs describe-services --cluster "$CLUSTER_NAME" --services "$ENVIOS_SERVICE_NAME" --query 'services[0].status' --output text 2>/dev/null || true)"
if [[ "$ENVIOS_STATUS" == "ACTIVE" ]]; then
  aws ecs update-service \
    --cluster "$CLUSTER_NAME" \
    --service "$ENVIOS_SERVICE_NAME" \
    --task-definition "$ENVIOS_TASK_DEFINITION_ARN" \
    --desired-count 1 \
  ...
  aws ecs create-service \
    --cluster "$CLUSTER_NAME" \
    --service-name "$ENVIOS_SERVICE_NAME" \
    --task-definition "$ENVIOS_TASK_DEFINITION_ARN" \
    --desired-count 1 \
    --launch-type FARGATE \
    --network-configuration "$NETWORK_CONFIGURATION" \
    --service-registries "registryArn=${ENVIOS_DISCOVERY_SERVICE_ARN}" >/dev/null
fi
...
aws ecs wait services-stable --cluster "$CLUSTER_NAME" --services "$ENVIOS_SERVICE_NAME" "$KONG_SERVICE_NAME"
