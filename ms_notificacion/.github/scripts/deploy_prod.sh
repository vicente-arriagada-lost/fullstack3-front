...
patch_notificaciones_task_definition() {
  jq -c \
  ...
  jq -c \
    ...
    --arg canary_weight "$canary_weight" '
      .containerDefinitions |= map(
        if .name == $container then
          .environment = ((.environment // [])
            | map(select(.name != "KONG_NOTIFICACIONES_STABLE_WEIGHT" and .name != "KONG_NOTIFICACIONES_CANARY_WEIGHT"))
            + [
                {"name":"KONG_NOTIFICACIONES_STABLE_WEIGHT","value":$stable_weight},
                {"name":"KONG_NOTIFICACIONES_CANARY_WEIGHT","value":$canary_weight}
              ])
        else
          .
        end
...
smoke_test_canary_route() {
  ...
  status_code="$(curl -sS -o /dev/null -w '%{http_code}' \
    ...
    --connect-timeout 5 \
    --max-time 15 \
    "http://${alb_dns}/api/notificaciones")"

  if [[ "$status_code" != "200" ]]; then
    echo "Smoke test failed. Expected 200 from notificaciones service, got ${status_code}." >&2
    exit 1
  fi
}
...
STABLE_SERVICE_NAME="$(get_param "$NOTIFICACIONES_PREFIX" service_name)"
CANARY_SERVICE_NAME="$(get_param "$NOTIFICACIONES_PREFIX" canary_service_name)"
NOTIFICACIONES_CONTAINER_NAME="$(get_param "$NOTIFICACIONES_PREFIX" container_name)"
NOTIFICACIONES_TASK_FAMILY="$(get_param "$NOTIFICACIONES_PREFIX" task_definition_family)"
CANARY_DISCOVERY_SERVICE_ARN="$(get_param "$NOTIFICACIONES_PREFIX" canary_discovery_service_arn)"
QUEUE_URL="$(get_param "$NOTIFICACIONES_PREFIX" queue_url)"
...
PATCHED_NOTIFICACIONES_TASK_DEFINITION_JSON="$(patch_notificaciones_task_definition <<<"$BASE_STABLE_TASK_DEFINITION_JSON")"
CURRENT_STABLE_IMAGE="$(jq -r --arg container "$NOTIFICACIONES_CONTAINER_NAME" '.containerDefinitions[] | select(.name == $container) | .image' <<<"$BASE_STABLE_TASK_DEFINITION_JSON")"

if [[ "$CURRENT_STABLE_IMAGE" == "alpine:latest" ]]; then
  STABLE_REGISTER_PAYLOAD="$(build_register_payload "$PATCHED_NOTIFICACIONES_TASK_DEFINITION_JSON" "$NOTIFICACIONES_TASK_FAMILY")"
...
CANARY_REGISTER_PAYLOAD="$(build_register_payload "$PATCHED_NOTIFICACIONES_TASK_DEFINITION_JSON" "notificaciones-canary-main")"
